...
function gifDisplay() {
  ...
  $(".results").append(
    '<h2 class="result_title">Now Playing: <span>' +
      $(this).data("name") +
      " Gifs!</span></h2>"
  );
  ...
  $.ajax({
    ...
    for (var i = 0; i < 12; i++) {
      ...
      gifDiv.append(
        '<img class="gifImg" src=' +
          gif[i].images.fixed_height_still.url +
          " data-still=" +
          gif[i].images.fixed_height_still.url +
          " data-animate=" +
          gif[i].images.fixed_height.url +
          ' data-state="still"/>'
      );
      gifDiv.append(
        '<span class="label label-default">Rated: ' + gif[i].rating + "</span>"
      );
